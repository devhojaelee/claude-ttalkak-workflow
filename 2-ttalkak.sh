...
PROJECT_ROOT="$(cd "$SCRIPT_DIR/../.." && pwd)"  # submodule이므로 2단계 위로
cd "$PROJECT_ROOT"

# .claude-config 로드
if [ ! -f .claude-config ]; then
    echo "❌ .claude-config 파일이 필요합니다"
    echo "템플릿을 복사하세요: cp .claude-core/templates/.claude-config.example .claude-config"
    exit 1
fi

source .claude-config

# ENV_FILE에서 LINEAR_API_KEY 로드
if [ -f "$ENV_FILE" ]; then
    set -a
    source "$ENV_FILE"
    set +a
...
if [ -z "$LINEAR_API_KEY" ]; then
    echo "❌ LINEAR_API_KEY가 설정되지 않았습니다."
...
linear mcp's team = ${LINEAR_TEAM}, project name = ${LINEAR_PROJECT}
...
---
### # Phase 4. Git Commit & Push + Project Validation (커밋/푸시 및 프로젝트 검증)
**CRITICAL**: 구현 완료 후 반드시 커밋, 검증, 푸시를 수행해야 합니다!

1. **변경사항 확인 및 Git Commit**:
   - git status로 변경된 파일 확인
   - 구현과 관련된 파일만 선택적으로 add
   - \`git commit -m "feat: [Issue-ID] 구현 내용 한 줄 요약"\`
   - 커밋 메시지는 명확하고 간결하게 작성
   - \`git log -1\`로 커밋 확인

2. **프로젝트별 검증** (실패 시 수정 후 재시도):
...
3. **Git Push** (필수):
   ...
   - Sub 브랜치는 백업 및 공유를 위해 반드시 push
   - Push 성공 확인: "Branch <branch-name> set up to track..." 메시지

---
### # Phase 5. Issue Management & Lock Release (이슈 관리 및 잠금 해제)
1. 구현에 성공했을 경우:
   - **Git 작업 검증**: Phase 4에서 commit과 push가 완료되었는지 확인
   - status = 'In Review'로 변경
   - comment에 다음 정보 포함:
     * 구현 내용 간결한 요약
     * 커밋 해시 (git log -1 --format=%H)
     * "✅ Commit & Push 완료" 명시
   - Lock 해제 (필수): Phase 1에서 사용된 모든 Lock 레이블 제거
   - "Sub 브랜치 구현 완료, commit & push 완료, Parent 브랜치에서 통합 대기 중" 추가
2. 구현에 실패했을 경우:
   - status = 'To Do'로 변경
   - Lock 유지
   - comment에 상세한 실패 분석 보고서 작성, 'failed' 레이블 추가
EOF
